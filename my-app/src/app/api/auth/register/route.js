...
export async function POST(req) {
    try {
        ...
        // Validation based on student year - no domain restrictions for project selection
        // Any category without projects gets auto-generated project ID

        if (isY24Student) {
            // Y24 students must select club and domain
            if (!selectedClub || !selectedDomain) {
                return NextResponse.json(
                    { message: "Club and domain selection is required for Y24 students" },
                    { status: 400 }
                );
            }

            // All domains require project selection (can be auto-generated from category if no projects exist)
            if (!selectedProject || !selectedCategory) {
                return NextResponse.json(
                    { message: "Project selection is required (category will be used if no projects exist)" },
                    { status: 400 }
                );
            }
