...
export default function FacultyAttendanceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    fetch(`/api/dashboard/faculty/attendance/${id}`)
      .then(r => r.json())
      .then(d => {
        if (d.submission) {
  ...
  const handleVerify = async (status: "verified" | "rejected") => {
    ...
    setVerifying(true);
    try {
      const res = await fetch(`/api/dashboard/faculty/attendance/${id}`, {
        method: "POST",
  ...
  if (!submission) return (
    <div className="max-w-5xl mx-auto p-6 text-center">
      ...
      <Link href="/dashboard/faculty/attendance" className="text-xs text-blue-600 hover:underline mt-2 inline-block">← Back to list</Link>
    </div>
  );
        ...
        <div className="p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              <Link href="/dashboard/faculty/attendance" className="p-2 bg-gray-100 rounded-full hover:bg-gray-200">
                <FiArrowLeft size={14} />
              </Link>
              <div>
