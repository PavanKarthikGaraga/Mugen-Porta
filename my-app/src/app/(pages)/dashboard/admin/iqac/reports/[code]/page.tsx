...
export default function IqacActivityReportFormPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/dashboard/iqac/activities/${code}`),
      fetch(`/api/dashboard/iqac/reports/${code}`)
    ]).then(async ([actRes, repRes]) => {
      ...
      const timeSlot = actData.activity.start_time && actData.activity.end_time
          ? `${String(actData.activity.start_time).slice(0, 5)} to ${String(actData.activity.end_time).slice(0, 5)}`
          : "";

      if (repData.report && repData.report.id) {
        ...
        setForm({
          ...
          studentsParticipated: repData.report.students_participated ?? "",
          posterUrl: repData.report.poster_url || "",
          permissionLetterUrl: repData.report.permission_letter_url || "",
          overview: repData.report.overview || "",
          objectives: repData.report.objectives || "",
          proceedings: repData.report.proceedings || "",
          keyHighlights: repData.report.key_highlights || "",
          learningOutcomes: repData.report.learning_outcomes || "",
          conclusion: repData.report.conclusion || "",
          gallery: repData.report.gallery || [],
          attendanceSheets: repData.report.attendance_sheets || [],
        });
        if ((repData.report.attendance_sheets || []).length > 0) setShowAttendanceSlot(false);
      } else {
        setForm((prev) => ({
          ...prev,
          timeSlot,
          venue: actData.activity.venue || "",
        }));
      }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        <Link href="/dashboard/admin/iqac/reports" className="p-2 bg-gray-100 rounded-full hover:bg-gray-200">
          <FiArrowLeft />
        </Link>
        <div>
          <h1 className="text-xl font-bold text-gray-900">{activity.title}</h1>
          <p className="text-xs text-gray-500 mt-0.5">
            Event Date: {formatActivityDate(activity.activity_date)}
      ...
      <section className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
        <h2 className="text-sm font-bold text-gray-900">Custom Signatures & Entity</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Organizing Entity</label>
            <input
              value={form.organizingEntity}
              onChange={(e) => setForm({ ...form, organizingEntity: e.target.value })}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Director Title</label>
            <input
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Faculty Title</label>
            <input
      ...
      <section className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
        ...
        <p className="text-[11px] text-gray-400">Exactly {MAX_GALLERY} photos required.</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
